...
const Reset = ({ location = {}, resetToken = '' }) => {
  ...
  const [failedPasswordChange, setPasswordChangeAttempt] = useState({
    status: false,
    message: ''
  });
  ...
  const handleSubmit = async e => {
    ...
    const data = formData;
    try {
      const checksOut = await fetch(`/api/reset`, {
        method: 'POST',
        headers: {
      ...
      if (res.action === 'error') {
        ...
        return setPasswordChangeAttempt({
          status: true,
          message: 'New password attempt failed. Please try again.'
        });
      }
  ...
  return (
    ...
    <Layout className="relative bg-gray-100 h-full" page={location.name || 'PickaFoodMood'}>
      <Flash />
      <h1 className="text-blue-100 shadow-md headingPop text-5xl sm:text-6xl flex justify-center m-5 tracking-wide">
        Reset Your Password
      </h1>
      {failedPasswordChange.status ? <FailedAttempt message={failedPasswordChange.message} /> : ''}
      <div className="flex justify-center w-full">
        ...
        >
          <div className="mb-4">
            ...
            <div className="fixWidthPw text-gray-600 bg-gray-100 text-xs m-auto mb-6 rounded">
      ...
      <style jsx>{`
        .headingPop {
          background: linear-gradient(135deg, #097cf6, #4dd9ff);
          margin: 0 0 1.25rem;
          padding: 2rem 0 2rem;
          font-weight: 700;
