...
const cookieEaten = `flash=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/;`;

const getCookie = cookieName => {
  const name = `${cookieName}=`;
  const ca = document.cookie.split(';');
  // const t = ca.filter(c => c.startsWith('flash'));
  document.cookie = cookieEaten;
...
const Flash = ({ cookies = '' }) => {
  ...
  useEffect(() => {
    ...
    if (flash) {
      const {
        flash: { type, text }
      } = JSON.parse(decodeURIComponent(flash));
      setType(type);
      setMessage(text);
      setTimeout(() => {
        setVisibility(true);
  ...
  return (
    <div className="max-w-lg m-auto relative transition-all">
      <div
        className={`${flashType === 'success' ? successBanner.container : errorBanner.container} ${
          visibility ? 'showMe' : 'hideMe'
        } m-auto flash w-full border-t-4 rounded-b px-4 py-3 shadow-md absolute transition ease-in-out duration-500`}
        role="alert"
      >
